...
TEST(DistributionTest, GaussianDistribution) {
  ...
  //   total_probability += Gaussian(mean, variance).density(x) *
  //   discretization;
  // }
  ...
  //   gaussian.updateMean(std::bind(&WelfordsAlgorithm::mean,
  //   std::ref(estimator), sample, std::placeholders::_1));
  //   gaussian.updateCovariance(
  //       std::bind(&WelfordsAlgorithm::variance, std::ref(estimator), sample,
  //       std::placeholders::_1));
  // }
...
TEST(DistributionTest, MultivariateGaussianDistribution) {
  ...
  // EXPECT_DOUBLE_EQ(probability_density, 1.0 / std::sqrt(std::pow(2 * M_PI,
  // 2)));
  ...
  //     Vector2d sample;
  //     sample << x, y;
  //     total_probability += Gaussian(mean, covariance).density(sample) *
  //     std::pow(discretization, 2);
  //   }
  // }
  // EXPECT_NEAR(total_probability, 1.0, 1e-6);
}